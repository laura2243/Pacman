...
class PositionSearchProblem(search.SearchProblem):
    ...
    def __init__(self, gameState, costFn=lambda x: 1, goal=(7, 10), start=None, warn=True, visualize=True):
        """
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if problem.isGoalState(state):
        ...
        for index, item in enumerate(state[1]):
            if item == False:  # Not visited corner
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    return 0 if len(foodGrid.asList()) == 0 else \
        min([mazeDistance(position, item,  problem.startingGameState) for item in foodGrid.asList()])
